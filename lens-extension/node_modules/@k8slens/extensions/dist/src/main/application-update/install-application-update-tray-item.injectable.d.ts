declare const installApplicationUpdateTrayItemInjectable: import("@ogre-tools/injectable").Injectable<{
    id: string;
    parentId: null;
    orderNumber: number;
    label: import("mobx").IComputedValue<string>;
    enabled: import("mobx").IComputedValue<boolean>;
    visible: import("mobx").IComputedValue<boolean>;
    click: () => void;
}, import("../tray/tray-menu-item/tray-menu-item-injection-token").TrayMenuItem, void>;
export default installApplicationUpdateTrayItemInjectable;
