...
export declare type InferParam<T extends string, PathParams extends Record<string, string>> = T extends `{${infer P}?}` ? PathParams & Partial<Record<P, string>> : T extends `{${infer P}}` ? PathParams & Record<P, string> : PathParams;
export declare type InferParamFromPath<P extends string> = P extends `${string}/{${infer B}*}${infer Tail}` ? Tail extends "" ? Record<B, string> : never : P extends `${infer A}/${infer B}` ? InferParam<A, InferParamFromPath<B>> : InferParam<P, {}>;
...
export declare type RouteResponse<Response> = LensApiResult<Response> | void;
...
export declare function route<Path extends string>(parts: BaseRoutePaths<Path>): BindHandler<Path>;
...
export declare function clusterRoute<Path extends string>(parts: BaseRoutePaths<Path>): BindClusterHandler<Path>;
...
export declare function payloadValidatedClusterRoute<Path extends string, Payload>({ payloadValidator, ...parts }: ValidatorBaseRoutePaths<Path, Payload>): BindValidatedClusterHandler<Path, Payload>;
