...
export declare type KubeJsonApiDataFor<K> = K extends KubeObject<infer Metadata, infer Status, infer Spec> ? KubeJsonApiData<Metadata, Status, Spec> : never;
export interface KubeObjectConstructorData {
    readonly kind?: string;
    readonly namespaced?: boolean;
    readonly apiBase?: string;
}
export declare type KubeObjectConstructor<K extends KubeObject, Data> = (new (data: Data) => K) & KubeObjectConstructorData;
...
export declare type KubeTemplateObjectMetadata<Namespaced extends KubeObjectScope> = Pick<KubeJsonApiObjectMetadata<KubeObjectScope>, "annotations" | "finalizers" | "generateName" | "labels" | "ownerReferences"> & {
    name?: string;
...
export declare type KubeJsonApiObjectMetadata<Namespaced extends KubeObjectScope = KubeObjectScope> = BaseKubeJsonApiObjectMetadata<Namespaced> & (Namespaced extends KubeObjectScope.Namespace ? {
    readonly namespace: string;
} : {});
export declare type KubeObjectMetadata<Namespaced extends KubeObjectScope = KubeObjectScope> = KubeJsonApiObjectMetadata<Namespaced> & {
    readonly selfLink: string;
    readonly uid: string;
    readonly name: string;
    readonly resourceVersion: string;
};
export declare type NamespaceScopedMetadata = KubeObjectMetadata<KubeObjectScope.Namespace>;
export declare type ClusterScopedMetadata = KubeObjectMetadata<KubeObjectScope.Cluster>;
...
export declare function isKubeStatusData(object: unknown): object is KubeStatusData;
export declare class KubeStatus {
...
export declare type KubeMetaField = keyof KubeJsonApiObjectMetadata;
...
export declare enum KubeObjectScope {
    Namespace = 0,
    Cluster = 1
}
export declare type ScopedNamespace<Namespaced extends KubeObjectScope> = (Namespaced extends KubeObjectScope.Namespace ? string : Namespaced extends KubeObjectScope.Cluster ? undefined : string | undefined);
export declare class KubeObject<Metadata extends KubeObjectMetadata<KubeObjectScope> = KubeObjectMetadata<KubeObjectScope>, Status = unknown, Spec = unknown> implements ItemObject {
    ...
    status?: Status;
    spec: Spec;
    static create<Metadata extends KubeObjectMetadata = KubeObjectMetadata, Status = unknown, Spec = unknown>(data: KubeJsonApiData<Metadata, Status, Spec>): KubeObject<Metadata, Status, Spec>;
    static isNonSystem(item: KubeJsonApiData | KubeObject<KubeObjectMetadata<KubeObjectScope>, unknown, unknown>): boolean;
     ...
     */
    delete(params?: object): Promise<KubeJsonApiData<KubeJsonApiObjectMetadata<KubeObjectScope>, unknown, unknown>>;
}
