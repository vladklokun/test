/// <reference types="react" />
declare const eventsRouteComponentInjectable: import("@ogre-tools/injectable").Injectable<{
    route: {
        path: string;
        clusterFrame: boolean;
        isEnabled: import("mobx").IComputedValue<boolean>;
    };
    Component: import("react").FunctionComponent<import("./events").EventsProps>;
}, {
    route: import("../../../common/front-end-routing/front-end-route-injection-token").Route<unknown>;
    Component: import("react").ElementType<any>;
}, void>;
export default eventsRouteComponentInjectable;
